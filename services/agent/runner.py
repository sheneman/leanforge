...
def run_turn(session_id: str) -> dict:
    ...
    if action == "SIMPLIFY":
        ...
        db.emit_event(session_id, "synthesize_result", {"tactics": leanstral_tactics})
    else:
    ...
    db.emit_event(session_id, "verify_start", {"source": source})
    t0 = time.time()
    ...
    if suggested_source:
        ...
        db.emit_event(session_id, "verify_start", {"source": suggested_source})
        t0 = time.time()
    ...
    if not result.get("success"):
        ...
        if has_unknown:
            ...
            if fixed_source != source:
                db.emit_event(session_id, "verify_start", {"source": fixed_source})
                t0 = time.time()
...
def run_loop(session_id: str, max_turns: int = 1000, delay: int = TURN_DELAY_SECS) -> None:
    ...
    for attempt in range(MAX_FORMALIZE_RETRIES + 1):
        ...
        error_msgs = [d.get("message", "") if isinstance(d, dict) else str(d) for d in real_errors[:3]]
